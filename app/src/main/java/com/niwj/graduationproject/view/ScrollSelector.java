...
import com.niwj.graduationproject.control.DisplayUtil;
...
public class ScrollSelector extends View {
     ...
     * 设置选择的监听器
     */
    public void setOnItemSelectorListener(OnItemSelectorListener l){
        mListener = l;
    }
    ...
    public void setTextSize(float size){
        size = DisplayUtil.sp2px(getContext(), size);
    ...
    private final int DEFAULT_TEXTSIZE = 23;    //默认字体大小,sp
    ...
    private int textSelectorColor = 0xFF3791DD;     //选中状态文字的颜色
    private int textNormalColor = 0xFFC0C0C0;       //正常状态文字的颜色
    private int marqueeX;                           //跑马灯的x坐标偏移
    private int marqueeWidth;                       //跑马灯的宽度
    private int borderWhenDown;                     //按下时的边界状态
    private int lastIndex;                          //上一个选择的序号
    ...
    private boolean isRollAble;                     //是否支持滚动手势
    ...
    private OnItemSelectorListener mListener;       //选择的回调接口

    public ScrollSelector(Context context, AttributeSet attrs) {
        super(context, attrs);
        mPaint = new Paint(Paint.ANTI_ALIAS_FLAG);                          //实例化画笔
        float textSize = DisplayUtil.sp2px(context, DEFAULT_TEXTSIZE);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < showItemNum + 1; i++){
            ...
            if (isFirst) {
                ...
                //选择回调
                if (mListener != null) {
                    if (lastIndex != getSelectedIndex()) {
                        lastIndex = getSelectedIndex();
                        mListener.onItemSelector(this, lastIndex);
                    }
                }
            }
        }
    ...
    private class RollThread extends Thread{
        ...
        private final float DAMPING = 0.2f;    //速度的衰减，即每一帧之后的衰减量
    ...
    public interface OnItemSelectorListener{
        void onItemSelector(View v, int position);
    }
}
